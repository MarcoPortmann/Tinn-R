...
uses
  ...
  ComCtrls, ToolWin, ExtCtrls, Menus, ActnList,
  System.Actions;
...
type
 ...
 //   synPP: TSynEditPrintPreview;
    ...
    procedure udZoomMouseDown(Sender: TObject; Button: TMouseButton;
...
begin
{  with synPP do
    if (SynEditPrint.Font.Size < 20) then
    begin
...
begin
  {with synPP do
    if (SynEditPrint.Font.Size > 04) then
    begin
      SynEditPrint.Font.Size := SynEditPrint.Font.Size - 2;
      frmConfigurePrint.iFontSize := SynEditPrint.Font.Size;
      UpdatePreview;
...
procedure TfrmPrintPreview.synPPPreviewPage(Sender: TObject;
  PageNumber: Integer);
begin
  //panPage.Caption := IntToStr(PageNumber) + '/' + IntToStr(synPP.PageCount);
...
{  frmTinnMain.iZoomPreview := synPP.ScalePercent;
  case synPP.ScaleMode of
    pscPageWidth:
      frmTinnMain.iScaleMode := 0;
    pscUserScaled:
      frmTinnMain.iScaleMode := 1;
    pscWholePage:
      frmTinnMain.iScaleMode := 2;
  end;
  Close;   }
end;
...
{  case frmTinnMain.iScaleMode of
    ...
    1:
      begin // pscUserScaled
        ...
        udZoom.Hint := 'Zoom = ' + IntToStr(synPP.ScalePercent) + '%';
...
procedure TfrmPrintPreview.FormMouseWheelUp(Sender: TObject; Shift: TShiftState;
  MousePos: TPoint; var Handled: Boolean);
begin
 // synPP.ScrollVertFor(20);
end;
