...
uses
  ...
  Buttons, Mask, ActnList, System.Actions, DScintillaTypes, DScintilla;
...
//    procedure bbtPreviewClick(Sender: TObject);
  ...
  public
    ...
    procedure ShowDialog(const seEditor: TDScintilla);
...
begin
 { prnOptions := TSynEditPrint.Create(Self);
  ...
  begin
    ...
    if actPageNumber.Checked then
      prnOptions.Header.Add('$PAGENUM$/$PAGECOUNT$', aFont, taRightJustify, 1);
  end;
end;
...
procedure TfrmConfigurePrint.ShowDialog(const seEditor: TDScintilla);
begin
...
{  FreeAndNil(prnOptions);
  ...
  begin
    ...
    if actLineWrap.Checked then
      iPrintLineWrap := SC_WRAP_WORD
...
begin
  ...
  begin
    ...
    actLineWrap.Checked := (iPrintLineWrap <> SC_WRAP_WORD );
...
procedure TfrmConfigurePrint.edStartPageKeyPress(Sender: TObject;
  var Key: Char);
begin
...
begin
 { with actLineNumber do
    Checked := not Checked;
  if Assigned(frmPrintPreview) then
  begin
    prnOptions.LineNumbers := actLineNumber.Checked;
    frmPrintPreview.synPP.UpdatePreview;
  end;   }
...
begin
  ...
  try
    ...
    raise Exception.Create('Error opening the Printer Configuration Dialog.' +
      #13 + #13 + 'Please, check if the printer is ok!');
  end;
end;
 ...
 { LoadOptions(True);
  ...
  begin
    if rbSelection.Checked then
      prnOptions.SelectedOnly := True
    else
      prnOptions.SelectedOnly := False;
    try
      prnOptions.Print;
    except
      // TO DO
