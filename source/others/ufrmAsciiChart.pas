...
const
  DescLow: array [0 .. 31] of string = ('Null', 'Start of Header',
    'Start of Text', 'end of Text', 'end of Transmission', 'Enquiry',
    'Acknowledgement', 'Bell', 'Backspace', 'Horizontal Tab', 'Linefeed',
    ...
    'Cancel', 'end Message', 'Sub', 'Escape', 'Form Separator',
...
begin
  ...
  begin
    ...
    begin
      X := I div 16;
      Y := I mod 16;
      Canvas.TextOut(X * HorizMult + 2, Y * VertMult + 28,
        IntToHex(BaseNum + I, 2));
    end;
  end
  else
  begin
  ...
  begin
    ...
    DrawCharacter(21, 'NAK', HorizMult, VertMult);
  ...
  for I := 0 to 7 do
    for J := 0 to 15 do
      Canvas.Polyline([Point(I * HorizMult, (J + 1) * VertMult + 24),
        Point(I * HorizMult, J * VertMult + 25), Point((I + 1) * HorizMult - 1,
        J * VertMult + 25)]);
...
procedure TfrmAsciiChart.DrawCharacter(const CharValue: Integer;
  const CharText: string; const HorizMult, VertMult: Integer);
var
  X, Y: Integer; { Screen Locations }
  MyRect: TRect; { general drawing reectangle }
  VOffset, HOffset: Integer;
  { V and H offsets for bounding box of character in font }
begin
  X := CharValue div 16;
  Y := CharValue mod 16;
  ...
  Canvas.TextRect(MyRect, MyRect.Left + HOffset, MyRect.Top + VOffset,
    CharText);
end;
...
procedure TfrmAsciiChart.FormMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  HorizMult, VertMult: Integer; { logical screen width/height segments }
...
begin
  ...
  if Button = MbRight then
    PopupMenu1.Popup(X + Left + GetSystemMetrics(SM_CXFRAME),
      Y + Self.Top + GetSystemMetrics(SM_CYFRAME) +
...
begin
  ...
  case NewFontSize of
    8:
      FontSizeUpDown.Position := 8;
    10:
      FontSizeUpDown.Position := 10;
    12:
      FontSizeUpDown.Position := 12;
  else
    Exit;
  end;
  if NewFontSize = FontSize then
    Exit; { No change so no need to redraw }
...
begin
  ...
  case FontSize of
    8:
      FontSize8.Checked := True;
    10:
      FontSize10.Checked := True;
    12:
      FontSize12.Checked := True;
  end;
...
begin
  ...
  begin
    ...
    PtMinTrackSize.Y := 331 + 2 * GetSystemMetrics(SM_CYFRAME) +
      GetSystemMetrics(SM_CYCAPTION);
  end;
  Msg.Result := 0;
end;
...
var
  I: Integer;
begin
  if FontComboName.Text = NewFontName then
    Exit;
...
procedure TfrmAsciiChart.FormMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
var
  HorizMult, VertMult: Integer; { logical screen width/height segments }
  XPos, YPos: Integer; { X and Y cells clicked on }
...
begin
  ...
  begin
    ...
    ThePoint := ClientToScreen(Point((XPos + 1) * HorizMult - 1,
      (YPos + 1) * VertMult + 24));
...
procedure TfrmAsciiChart.FormMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
var
  HorizMult, VertMult: Integer; { logical screen width/height segments }
  XPos, YPos: Integer; { X and Y cells clicked on }
...
begin
  ...
  if Assigned(TfrmEditor(frmTinnMain.MDIChildren[0])) then
    with TfrmEditor(frmTinnMain.MDIChildren[0]).sciEditor do
      if (AsciiString <> '') then
      begin
        ReplaceSel(AsciiString);
      end;
end;
