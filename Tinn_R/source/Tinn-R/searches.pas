...
begin
  if fCaseSensitive = CaseSens then
    Exit;
...
begin
  ...
  // ie: StartPos must never be less than fPatLen-1
    fPos := fStart + fPatLen - 1
  else
...
begin
  ...
  begin
    ...
    begin // last char matches at least
      i := 1;
      j := fPos - fPatLen;
      while (i < fPatLen) and (fPattern[i] = (j + i)^) do
        inc(i);
      if (i = fPatLen) then
      begin
...
begin
  ...
  begin
    ...
    begin // last char matches at least
      ...
      j := fPos - fPatLen;
      while (i < fPatLen) and
        (CaseBlindTable[fPattern[i]] = CaseBlindTable[(j + i)^]) do
        inc(i);
      if (i = fPatLen) then
      begin
        Result := fPos - fStart - fPatLen + 1;
...
begin
  ...
  filemappinghandle := CreateFileMapping(filehandle, nil, page_readonly,
    0, 0, nil);
