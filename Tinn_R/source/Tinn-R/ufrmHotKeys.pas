...
uses
  ...
  JvExComCtrls, JvHotKey, JvComCtrls, JvgPage, Graphics, Buttons, System.UITypes;
...
type
  TfrmHotKeys = class(TForm)
    ...
    procedure jvhkHotkeyKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure jvhkHotkeyKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    ...
    procedure strgCustomDrawCell(Sender: TObject; ACol, ARow: Integer;
      Rect: TRect; State: TGridDrawState);
    procedure strgCustomSelectCell(Sender: TObject; ACol, ARow: Integer;
      var CanSelect: Boolean);
    procedure strgDefaultClick(Sender: TObject);
    procedure strgDefaultDrawCell(Sender: TObject; ACol, ARow: Integer;
      Rect: TRect; State: TGridDrawState);
    procedure strgDefaultSelectCell(Sender: TObject; ACol, ARow: Integer;
      var CanSelect: Boolean);
...
begin
  for i := 1 to 20 do
    with frmTinnMain do
    begin
      if Assigned(ajavHK[i]) then
        ajavHK[i].Active := bStatus;
      if Assigned(ajavHKC[i]) then
        ajavHKC[i].Active := bStatus;
    end;
end;

procedure TfrmHotKeys.CreateHotkeyDefault(i: Integer; sTmp: string;
  iRow: Integer);
begin
  with frmTinnMain do
  begin
...
procedure TfrmHotKeys.CreateRCustom(i: Integer; sTmp: string; iRow: Integer);
begin
  with frmTinnMain do
    aRC[i] := sTmp;
...
procedure TfrmHotKeys.CreateHotkeyCustom(i: Integer; sTmp: string;
  iRow: Integer);
begin
  with frmTinnMain do
  begin
...
begin
  ...
  begin
    for i := 1 to 20 do
    begin
      aHK[i] := ifHotKeys.ReadString('R Hotkeys', 'RHK' + IntToStr(i), '');
      if (aHK[i] <> '') then
      begin
        sTmp := aHK[i];
        CreateHotkeyDefault(i, sTmp, i);
      end;
    end;
  end;
  ...
  begin
    for i := 1 to 20 do
    begin
      aHK[i] := ifHotKeys.ReadString('R Custom', 'RC' + IntToStr(i), '');
      if (aHK[i] <> '') then
      begin
        sTmp := aHK[i];
        CreateRCustom(i, sTmp, i);
      end;
    end;
  end;
  ...
  case pgHotkeys.TabIndex of
    0:
      begin
        ...
        CreateHotkeyDefault(iRow, sTmp, iRow);
      end;
    1:
      begin
        iRow := strgCustom.Row;
        ...
        sTmp := ShortCutToText(jvhkHotkey.HotKey);
        CreateRCustom(iRow, edHotkey.Text, iRow);
        CreateHotkeyCustom(iRow, sTmp, iRow);
      end;
  end;
end;
    ...
    0:
      begin
        ...
        try
          ...
          MessageDlg('Hotkey not defined yet!', mtInformation, [MBOK], 0);
        end;
      end;
    1:
      begin
...
begin
  ...
  for i := 1 to 20 do
  begin
    if (lowercase(strgDefault.Cells[1, i]) = lowercase(sTmp)) then
    begin
      with frmTinnMain do
      begin
        ...
        case i of
          1:
            if actRSendFile.Enabled then
              actRSendFileExecute(nil);
          2:
            if actRSendSelection.Enabled then
              actRSendSelectionExecute(nil);
          3:
            if actRSendClipboard.Enabled then
              actRSendClipboardExecute(nil);
          4:
            if actRSendBlockMarked.Enabled then
              actRSendBlockMarkedExecute(nil);
          5:
            if actRSendContiguous.Enabled then
              actRSendContiguousExecute(nil);
          6:
            if actRSendLine.Enabled then
              actRSendLineExecute(nil);
          7:
            if actRSendLinesToEndPage.Enabled then
              actRSendLinesToEndPageExecute(nil);
          8:
            if actRCurrentLineToTop.Enabled then
              actRCurrentLineToTopExecute(nil);
          9:
            if actRSendNavigator.Enabled then
              actRSendNavigatorExecute(nil);
          10:
            if actRContPrintVariableContent.Enabled then
              actRContPrintVariableContentExecute(nil);
          11:
            if actRContListVariableNames.Enabled then
              actRContListVariableNamesExecute(nil);
          12:
            if actRContListVariableStructure.Enabled then
              actRContListVariableStructureExecute(nil);
          13:
            actRContListAllObjectsExecute(nil);
          14:
            actRContClearConsoleExecute(nil);
          15:
            actRContCloseAllGraphicsExecute(nil);
          16:
            if actRContHelpSelectedWord.Enabled then
              actRContHelpSelectedWordExecute(nil);
        end;
...
begin
  ...
  for i := 1 to 20 do
  begin
    if (lowercase(strgCustom.Cells[1, i]) = lowercase(sHKC)) then
    begin
...
procedure TfrmHotKeys.AppMessage(var Msg: TMSG; var bHandled: Boolean);
begin
  bHandled := False;
  case Msg.Message of
    WM_SYSKEYDOWN:
      if (Msg.wParam = VK_F4) then
        bHandled := True; // don't allow ALT-F4
  end;
end;
...
procedure TfrmHotKeys.strgDefaultDrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
begin
  (*
    if (arow > 0)  then
...
procedure TfrmHotKeys.strgCustomDrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);
begin
  (*
    if (arow > 0)  then
...
procedure TfrmHotKeys.strgCustomSelectCell(Sender: TObject; ACol, ARow: Integer;
  var CanSelect: Boolean);
begin
  ClearWarnings;
  with frmTinnMain do
    if Assigned(ajavHKC[ARow]) then
      jvhkHotkey.HotKey := ajavHKC[ARow].HotKey
    else
      jvhkHotkey.HotKey := TextToShortCut('None');
...
begin
  ClearWarnings;
  with frmTinnMain do
    if Assigned(ajavHK[ARow]) then
      jvhkHotkey.HotKey := ajavHK[ARow].HotKey
    else
      jvhkHotkey.HotKey := TextToShortCut('None');
...
begin
  ...
  with frmTinnMain do
  begin
    if (rdgTinnToRHotKeys.ItemIndex = 1) then
      bTinnRHotKeysActive := True
    else
      bTinnRHotKeysActive := False;
...
begin
  ...
  with rdgTinnToRHotKeys do
    if (frmTinnMain.bTinnRHotKeysActive = True) then
      (ItemIndex := 1)
    else
      (ItemIndex := 0);

  if (pgHotkeys.TabIndex = 0) then
  begin
    iRow := strgDefault.Row;
    edHotkey.Text := strgDefault.Cells[0, iRow];
    with frmTinnMain do
      if Assigned(ajavHK[iRow]) then
        jvhkHotkey.HotKey := ajavHK[iRow].HotKey;
  end
  else
  begin
    iRow := strgCustom.Row;
...
procedure TfrmHotKeys.jvhkHotkeyKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfrmHotKeys.jvhkHotkeyKeyUp(Sender: TObject; var Key: Word;
  Shift: TShiftState);
var
  pTmp: pointer;
  iRow: Integer;
...
begin
  ...
  if (modDados.cdShortcuts.Locate('Shortcut',
    StringReplace(ShortCutToText(jvhkHotkey.HotKey), ' ', '', []), []) = True)
  then
  begin
    ...
    begin
      iRow := strgDefault.Row;
      if Assigned(frmTinnMain.ajavHK[iRow]) then
        jvhkHotkey.HotKey := frmTinnMain.ajavHK[iRow].HotKey
      else
        jvhkHotkey.HotKey := TextToShortCut('None');
    end
    else
    begin
      iRow := strgCustom.Row;
      if Assigned(frmTinnMain.ajavHKC[iRow]) then
        jvhkHotkey.HotKey := frmTinnMain.ajavHKC[iRow].HotKey
      else
        jvhkHotkey.HotKey := TextToShortCut('None');
    end;
  end
