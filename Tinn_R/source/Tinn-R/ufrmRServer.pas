...
type
  TfrmRServer = class(TForm)
    ...
    procedure memRTCPIPKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
begin
  ...
  begin
    if bIPLocal then
      rdgIPType.ItemIndex := 0
    else
      rdgIPType.ItemIndex := 1;
...
procedure TfrmRServer.SaveOptions;
begin
  with frmTinnMain do
  begin
    if (rdgIPType.ItemIndex = 0) then
      bIPLocal := True
    else
      bIPLocal := False;
...
begin
  ...
  begin
    ...
    if frmRterm.bRTerm_Running and bIPLocal then
    begin
...
begin
  ...
  with frmTinnMain do
  begin
      if (GetCurrentLine(frmRterm.sciIO) = '> ') then
        PrintMessage(sTmp, False)
      else
...

{
  ...
  begin
  if (Key = #13) then
  if frmTinnMain.UseTCPIP then btnSendTCPIPClick(Self);
  end;
}
...
begin
  if (rdgIPType.ItemIndex = 0) then
  begin
    frmTinnMain.bIPLocal := True;
    pgIP.ActivePage := tbsIPLocal
  end
  else
  begin
    frmTinnMain.bIPLocal := False;
    pgIP.ActivePage := tbsIPRemote;
  end;
end;

procedure TfrmRServer.edtSendKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (Key = VK_RETURN) then
  begin
    if frmTinnMain.UseTCPIP then
      btnSendTCPIPClick(Self);
    edtSend.SetFocus;
    edtSend.SelectAll;
  end;
...
procedure TfrmRServer.memRTCPIPKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if (Shift = [ssCtrl]) then
    case Key of
