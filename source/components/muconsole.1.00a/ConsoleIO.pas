...
uses Messages, Windows, SysUtils, Classes, Forms, vcl.dialogs,
  Controls, System.AnsiStrings, RegularExpressions;
...
procedure TConsoleIO.ReceiveOutput(Buf: Pointer; Size: Integer);
  ...
  function getPosSave(substr, str: String; var fnd: Integer): Boolean;
  begin
    fnd := Pos(substr, str);
    Result := fnd > 0;
  end;
...
{
 OutputBuffer := regexpr.Replace(OutputBuffer, '[^\r]\n', #13#10);
 iPos := System.AnsiStrings.LastDelimiter( #13#10, OutputBuffer);

  if iPos > 0 then
    iPos:= iPos - 1;
  if iPos > 0 then
  begin
    sOutput := sOutput + Copy(OutputBuffer, 1, iPos -1)+#13#10;
  ...
  while  getPosSave(#10, OutputBuffer, iPos) do
  begin
    if OutputBuffer[iPos-1] = #13 then
    begin
      iPos := iPos-1;
      iSplitterLength := 2;
    end else iSplitterLength := 1;

    //sOutput := sOutput + '^B^'+ Copy(OutputBuffer, 1, iPos -1)+#13#10+'^'+inttostr(iSplitterLength)+'^';
    sOutput := sOutput + Copy(OutputBuffer, 1, iPos -1)+#13#10;

    OutputBuffer := Copy(OutputBuffer, iPos + iSplitterLength, Length(OutputBuffer));
  end;

  if copy(sOutput, Length(sOutput)-1, 2) = #13#10  then
    sOutput := copy(sOutput, 1, Length(sOutput)-2);


  if sOutput = '>' then
    sOutput := '> ';
  FOnReceiveOutput(Self, sOutput);
  ...
  if (trim(OutputBuffer) = '>') or bUnderDebug then
  begin

    //
    //   There is probably something wrong above and the below is the best fix I could find so far.
    if trim(OutputBuffer) = '>' then  //
      OutputBuffer := '> ';           //
    //
