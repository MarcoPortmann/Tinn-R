...
function TActionMacroEvent.GetAsString: WideString;
begin
  Result := 'ecAction ' + FActionName;
  if RepeatCount > 1 then Result := Result + ' ' + IntToStr(RepeatCount);
end;

procedure TActionMacroEvent.InitEventParameters(sStr: WideString);
var
  cHead, cTail: PChar;
...
procedure TActionMacroEvent.Initialize(aCmd: TsynEditorCommand; aChar: WideChar; aData: Pointer);
begin
