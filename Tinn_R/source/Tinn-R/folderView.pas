...
begin
  ...
  with sgFolder1 do begin
    cells[0,0] := 'Name';
    cells[1,0] := 'Size';
    cells[2,0] := 'Modified';
  end;
  with sgFolder2 do begin
    cells[0,0] := 'Name';
    cells[1,0] := 'Size';
    cells[2,0] := 'Modified';
  end;
  ...
  sgFolder1.ColWidths[1] :=
    frmDiffMain.Canvas.TextWidth(format('    %1.0n KB',[1024 * 99000/1]));
...
begin
  ...
  if IsFolder1 then begin
    if not GetFolder(frmDiffMain,'Open folder 1',LastOpenedFolder1) then exit;
    DoOpenFolder(LastOpenedFolder1, true)
  end
  else begin
    if not GetFolder(frmDiffMain,'Open folder 2',LastOpenedFolder2) then exit;
...
begin
  if Sender = sgFolder1 then begin
  ...
  idx := ARow -1;
  with sgFolder, Canvas do begin

    if (ARow > 0) and (FolderStringList.Count = 0) then begin
      brush.Color := color;
      FillRect(Rect);
      exit;
    end;
    ...
    if ACol = 0 then begin
      ...
      if (idx < FolderStringList.Count) and not (gdFixed in State) then begin
...
begin
  ...
  for i := 0 to FolderStringList.Count -1 do
    with PFolderRec(FolderStringList.objects[i])^ do begin
...
begin
  ...
  frmDiffMain.mnuCompareFiles.Enabled :=
...
begin
  ...
  if assigned(fFolder1StringList.objects[i]) and
    (PFolderRec(fFolder1StringList.objects[i]).Difference = fdModified) then begin
      //toggle to fileview ...
      ...
      with TfrmFilesFrame(frmDiffMain.FilesFrame) do begin
        if (pnlCaptionLeft.Color = ISMODIFIED_COLOR) then begin
          case application.MessageBox(pchar('Save changes to'#10 +
            ...
            IDCANCEL: exit;
            IDYES: SaveFileClick(frmDiffMain.mnuSave1);
          end;
        end;
        if (pnlCaptionRight.Color = ISMODIFIED_COLOR) then begin
          case application.MessageBox(pchar('Save changes to'#10 +
            pnlCaptionRight.Caption+' ?  '),pchar(application.title),
              MB_YESNOCANCEL or MB_ICONSTOP or MB_DEFBUTTON3) of
            IDCANCEL: exit;
            IDYES: SaveFileClick(frmDiffMain.mnuSave2);
          end;
        end;
