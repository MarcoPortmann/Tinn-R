...
uses
  ...
  DScintillaCustom, DScintillaTypes, DScintilla;
...
begin
  ...
  end;
 {
  ...
  //if cRTerm.bRterm_Ready then
  //  if frmTinnMain.bTCPIPRunning then
    begin
      frmTinnMain.bRObjectsUpdate := true;
      //frmTinnMain.actREnvironmentRefresh.Execute;
      //frmTinnMain.actRExplorerRefresh.Execute;
    end;
//showmessage( inttostr(sciIO.GetStyleAt(sciIO.GetCurrentPos)));
end;
...
begin
  ...
  if Assigned(sciLog2) then
    sciLogA := sciLog2 { synIO }
  else
    sciLogA := sciLog { synIO };
...
begin
  ...
  //sci //m.p.
//  iWidth := (sciIO.Width - sciIO.GetMarginLeft - 20) div sciIO.SendEditor();
//  Int(STYLE_DEFAULT )
//
//  if (iWidth >= 10) then

  sToSend := 'options(''width''=' + IntToStr(10000 ) + ')' + #13#10;
...
begin
  if frmTinnMain.actRtermIOSplitHorizontal.Checked then
    iSciLog2Height := SciLog2.Height;
  if frmTinnMain.actRtermIOSplitVertical.Checked then
    iSciLog2Width := SciLog2.Width;
end;
  ...
  begin
    ...
    begin
      ...
      if (iSciLog2Height > tbsIO.Height) then
        SciLog2.Height := 4 * (tbsIO.Height div 5)
      else
        SciLog2.Height := iSciLog2Height
    end
    else
    begin
      Align := alRight;
      if (iSciLog2Width > tbsIO.Width) then
        SciLog2.Width := 2 * (tbsIO.Width div 3)
      else
        SciLog2.Width := iSciLog2Width
    end;
    Parent := tbsIO;
    BorderStyle := bsNone;
    //sci //m.p. HideSelection := False;
    OnKeyDown := SciLogKeyDown;
    PopupMenu := frmTinnMain.pmenLog;

    //sci UDATE !!!! //m.p.
    {Options := SciIO.Options;
    Gutter := SciIO.Gutter;
    Gutter.ShowlineNumbers := SciIO.Gutter.ShowlineNumbers;}
    {Font := SciIO.Font;
    Highlighter := SciIO.Highlighter;
    SelectionMode := SciIO.SelectionMode;
    Options := SciIO.Options;
    WantTabs := SciIO.WantTabs;
    ActiveLineColor := SciIO.ActiveLineColor;}
    {Color := SciIO.Color;
    TabWidth := SciIO.TabWidth;
    RightEdge := SciIO.RightEdge;
    RightEdgeColor := SciIO.RightEdgeColor;}
    OnKeyUp := SciLog.OnKeyUp;
    OnMouseUp := SciLog.OnMouseUp;
    with Constraints do
    begin
      MinHeight := 20;
      MinWidth := 30;
