...
begin
  ...
  else if FoldersFrame.Visible then
    mnuFolderClick(nil)
  else
...
begin
  with TIniFile.Create(frmTinnMain.sPathApp + '\Tinn.ini') do // JCFaria
    try
      ...
      addClr := strtointdef(ReadString('Options TextDiff', 'sAddColor',
        ''), $F0CCA8);
      modClr := strtointdef(ReadString('Options TextDiff', 'sModColor',
        ''), $6FFB8A);
      delClr := strtointdef(ReadString('Options TextDiff', 'sDelColor',
        ''), $BB77FF);

      // set font
      with TfrmFilesFrame(FilesFrame).fdFiles.Font do
      begin
        Name := ReadString('Options TextDiff', 'sFont.Name', 'Courier New');
        Size := ReadInteger('Options TextDiff', 'iFont.Size', 10);
        Color := strtointdef(ReadString('Options TextDiff', 'sFont.Color', ''),
          $00000000);
      end;

      if ReadBool('Options TextDiff', 'bHorizontal', false) then
        mnuHorzSplit.Checked := true;

      LastOpenedFolder1 := ReadString('Options TextDiff', 'sFolder.1', '');
      LastOpenedFolder2 := ReadString('Options TextDiff', 'sFolder.2', '');
      FoldersFrame.Visible := ReadBool('Options TextDiff',
        'bFolderView', false);
    finally
      free;
    end;
  ...
  with TIniFile.Create(frmTinnMain.sPathApp + '\Tinn.ini') do // JCFaria
    try
      ...
      begin
        ...
        WriteString('Options TextDiff', 'sFont.Color',
          '$' + inttohex(Color, 8));
      end;
...
begin
  ...
  if FileExists(pathHelp) then
  begin
    application.helpfile := pathHelp;
    application.HelpCommand(HELP_CONTENTS, 0);
  end
  else
    MessageDlg(pathHelp + #13 + #13 + 'The help file was not found.' + #13 +
      'Please, contact Tinn-R team!', mtWarning, [mbOK], 0);
end;
// ---------------------------------------------------------------------
