...
type
  TfrmRterm = class(TForm)
    ...
    procedure sciIOContextPopup(Sender: TObject; MousePos: TPoint;
      var Handled: Boolean);
  private
...
uses
  ...
  ufrmEditor,
  ufrmTools,
  trUtils, uRSendCommands, uLexerCommands, umodDados, RibbonMarkup;
...
if bDebug then
  ...
  begin
    ...
    begin
      // (Un)comment the lines below to get an empty line before '> '
...
procedure TfrmRterm.sciIOContextPopup(Sender: TObject; MousePos: TPoint;
  var Handled: Boolean);
begin
  frmTinnMain.Ribbon.ShowContextPopup(CmdMContextMenuConsole);
 // frmTinnMain.Ribbon.ShowContextPopup(CmdWebRNews);
end;
...
begin
  ...
  begin // Send latest line to R when editing related to Rterm
    ...
    if sciIO.AutoCActive then
      begin
        sciIO.AutoCComplete;
        Key :=  VK_Tab;  // VK_PAUSE doesn't work
