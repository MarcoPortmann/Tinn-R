...
procedure TfrmCount.strGridDrawCell(Sender: TObject; ACol, ARow: Integer;
  Rect: TRect; State: TGridDrawState);

  procedure WriteText(StringGrid: TStringGrid; ACanvas: TCanvas;
    const ARect: TRect; const Text: string; Format: Word);
  const
    iX = 2;
    iY = 2;
  var
  ...
  begin
    ...
    begin
      case Format of
        DT_LEFT:
          ExtTextOut(Handle, Left + iX, Top + iY, ETO_OPAQUE or ETO_CLIPPED,
            @ARect, StrPCopy(cTemp, Text), Length(Text), nil);

        DT_RIGHT:
          ExtTextOut(Handle, Right - TextWidth(Text) - 3, Top + iY,
            ETO_OPAQUE or ETO_CLIPPED, @ARect, StrPCopy(cTemp, Text),
            Length(Text), nil);

        DT_CENTER:
          ExtTextOut(Handle, Left + (Right - Left - TextWidth(Text)) div 2,
            Top + iY, ETO_OPAQUE or ETO_CLIPPED, @ARect, StrPCopy(cTemp, Text),
            Length(Text), nil);
      end;
    end;
  end;
...
begin
  if (ACol = 1) then
    Display(strGrid, strGrid.Cells[ACol, ARow], taRightJustify);
