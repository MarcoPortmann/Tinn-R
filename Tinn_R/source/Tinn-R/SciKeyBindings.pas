{
 ...
 History:   29/09/2004 Initial Release with Delphi Scintilla Interface Components
            ...
            07/09/2007 Added SCI_DELRIGHTEND (1.74 CVS 07/09/2007)
}
...
interface
Uses
  Windows, Classes, Controls, SciSupport, SciResLang, DScintilla;
...
//		(Value: SCI_DELWORDRIGHT; Name: sKeyDeleteWordRightEnd),
    ...
    (Value: SCI_SELECTIONDUPLICATE; Name: sSelectionDuplicate)
    );
...
Type
  ...
  private
    ...
    function GetItem(Index:Integer): TSciKeyCommand;
    procedure SetItem(Index:Integer; Value: TSciKeyCommand);
  protected
  public
    constructor Create(Editor : TWinControl);
    procedure Assign(Source: TPersistent); override;
    procedure Update(Item: TCollectionItem); override;
    function Add: TSciKeyCommand;
    ...
    property Items[Index : Integer] : TSciKeyCommand read GetItem write SetItem;default;
  end;
...
Uses
 { SciSupport,}Menus,SysUtils{,sciUtils};
...
begin
  case keyIn of
    ...
    VK_LWIN: Result:=SCK_WIN;
    VK_RWIN: Result:=SCK_RWIN;
    VK_APPS: Result:=SCK_MENU;
    else
      Result := keyIn;
  end;
end;
...
begin
  case keyIn of
    ...
    SCK_WIN: Result:=VK_LWIN;
    SCK_RWIN: Result:=VK_RWIN;
    SCK_MENU: Result:=VK_APPS;
    Ord(';'): Result:=VK_OEM_1;
    Ord('/'): Result:=VK_OEM_2;
    Ord('`'): Result:=VK_OEM_3;
    Ord('['): Result:=VK_OEM_4;
...
begin
  ...
  with (Item as TSciKeyCommand) do
    TDScintilla(fEditor).AssignCmdKey(ShortCutToSciKey(FShortCut), FCommand)
  else
  begin
    TDScintilla(fEditor).ClearAllCmdKeys;
    for i := 0 to Count - 1 do with (Items[i] as TSciKeyCommand) do
      TDScintilla(fEditor).AssignCmdKey(ShortCutToSciKey(FShortCut), FCommand);
  end;
end;

function TSciKeyCommandCollection.GetItem(Index:Integer): TSciKeyCommand;
begin
  Result:=TSciKeyCommand(inherited GetItem(Index));
end;

procedure TSciKeyCommandCollection.SetItem(Index:Integer; Value: TSciKeyCommand);
begin
  inherited SetItem(Index,Value);
end;

function TSciKeyCommandCollection.Add: TSciKeyCommand;
begin
  Result:=TSciKeyCommand(inherited Add);
end;
...
function TSciKeyCommandCollection.FindShortCut(ShortCut: TShortCut): TSciKeyCommand;
var
  i : Integer;
  KeyCommand : TSciKeyCommand;
begin
...
begin
  ...
  try
    ...
    AddCommandKey(Ord('D'),SCI_CTRL,SCI_SELECTIONDUPLICATE);
...
begin
  if Source is TSciKeyCommand then
  begin
