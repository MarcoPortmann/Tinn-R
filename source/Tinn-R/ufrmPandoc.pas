...
type
  ...
  public
    ...
    property PandocHistoryFrom: string read GetPandocHistoryFrom
      write SetPandocHistoryFrom;
    property PandocHistoryTo: string read GetPandocHistoryTo
      write SetPandocHistoryTo;
  end;
...
begin
  Result := '';
  for i := 0 to (cbPandocHistory.Items.Count - 1) do
  begin
    if (i >= 10) then
      break;
    if (i > 0) then
      Result := Result + #13#10;
    Result := Result + cbPandocHistory.Items[i];
  end;
end;
...
begin
  ...
  i := 0;
  while (slHistory.Count >= 1) and (i <= 10) do
  begin
...
begin
  Result := False;
  if (rdgFrom.ItemIndex <> -1) and (rdgTo.ItemIndex <> -1) then
    Result := True;
end;

procedure TfrmPandoc.cbPandocHistorySelect(Sender: TObject);
begin
  if (cbPandocHistory.Text = '') then
    Exit;
...
procedure TfrmPandoc.MakePandocInstruction(sInstruction, sFrom, sTo: string);
begin
  sPandocInstruction := StringReplace(sInstruction, '%from', sFrom, []);

  sPandocInstruction := StringReplace(sPandocInstruction, '%to', sTo, []);
end;

procedure TfrmPandoc.PandocDefaultInstruction(iFrom, iTo: integer);
  ...
  begin
    with frmTinnMain do
      with (MDIChildren[FindTopWindow] as TfrmEditor) do
        sTmp := EditorFile.sFile;
    Result := sTmp;
  end;

begin
  sPandocFrom := GetInputFile;
  sPandocTo := ChangeFileExt(sPandocFrom, '.' + GetName(rdgTo.Items[iTo]));
  edFrom.Text := sPandocFrom;
  edTo.Text := sPandocTo;

  edInstruction.Text := 'pandoc' + ' -f ' + GetName(rdgFrom.Items[iFrom]) +
    ' -t ' + GetName(rdgTo.Items[iTo]) + ' ' + '%from' + ' -o' + ' ' + '%to';

  MakePandocInstruction(edInstruction.Text, sPandocFrom, sPandocTo);
end;
...
begin
  ...
  begin
    ...
    PandocDefaultInstruction(iPandocFrom, iPandocTo);
  end;
end;

procedure TfrmPandoc.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
var
  sTmp: string;
  i: integer;

begin
  if (cbPandocHistory.Text <> '') and (edHistoryFrom.Text <> '') and
    (edHistoryTo.Text <> '') then
  begin

    sPandocFrom := edHistoryFrom.Text;
    sPandocTo := edHistoryTo.Text;

    MakePandocInstruction(cbPandocHistory.Text, sPandocFrom, sPandocTo);
    ...
    begin
      ...
      begin
        ...
        cbPandocHistory.Items.Insert(0, sTmp);
...
begin
  if not CanMakeInstruction then
    Exit;
...
begin
  ...
  PandocDefaultInstruction(rdgFrom.ItemIndex, rdgTo.ItemIndex);
end;
