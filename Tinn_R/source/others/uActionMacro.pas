...
uses
  ...
  Menus, StdActns, ActnList, StdCtrls, ExtCtrls,
  System.Actions;
 {
const
...
{type
  ...
  private
    ...
    fActionLists: TList;
  protected
    function GetAsStringAct: String; // m.p. override;
    procedure InitEventParametersAct(sStr: String); // m.p. override;
  public
...
function TActionMacroEvent.GetAsStringAct: String;
begin
...
procedure TActionMacroEvent.InitEventParametersAct(sStr: String);
var
  cHead, cTail: PChar;

begin
  cHead := PChar(sStr);
  while CharInSet(cHead^, [#6, ' ']) do
    Inc(cHead);
  cTail := cHead;
  while CharInSet(cTail^, ['A' .. 'Z', 'a' .. 'z', '0' .. '9', '_']) do
    Inc(cTail);
  fActionName := Copy(cHead, 1, cTail - cHead);
  RepeatCount := StrToIntDef(Trim(cTail), 1);
end;

procedure TActionMacroEvent.Initialize(aCmd: TsynEditorCommand; aChar: WideChar;
  aData: Pointer);
begin
